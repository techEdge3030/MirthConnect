...
Set-Location "C:\Projects\Dev_MirthAdmin\mirthWebApp\client"

# Initialize log file
$logPath = Join-Path (Get-Location) $LogFile
Write-TimestampedLog "Starting npm run dev..." $logPath

# Start npm run dev process
$process = Start-Process -FilePath "npm" -ArgumentList "run", "dev" -NoNewWindow -PassThru -RedirectStandardOutput "temp-stdout.txt" -RedirectStandardError "temp-stderr.txt"
